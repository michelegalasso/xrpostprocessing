...
In this document I present a series of Python scripts used to postprocess the results of USPEX calculations when working with experimental X-ray diffraction data. The core of this collection of scripts is constituted by the scripts \texttt{powder\_xrd\_screening.py} and \texttt{scxrd\_screening.py}, which allow the user to quickly sort a number of structures read from the USPEX output according to their degree of agreement with experimental X-ray data. The first script works with powder XRD data, while the second script works with single crystal XRD data. Both scripts can be used not only with USPEX output files, but also with any set of structures in the POSCAR format, for example, with structures taken from a database. The theory beyond the functioning of these two scripts is illustrated in appendices \ref{sec:powderspectracomp} and \ref{sec:scspectracomp}.

The first 3 scripts take in input a number of files written by USPEX, while the remaining scripts work with structural information contained in CIF or POSCAR files. All script take in input some information about the experimental X-ray data as well. At the time of writing this document, the collection of scripts has been tested on the following Python versions: 3.6, 3.7, 3.8, 3.13.
...
The script screens a collection of structures to identify those that best match an experimental powder X-ray diffraction pattern. For the theory behind this script, see appendix \ref{sec:powderspectracomp}. In a few words, given an experimental pattern in input, the theoretical diffraction pattern is calculated for each candidate structure and a value F is computed. The smaller is F, the better is the agreement between theoretical and experimental patterns. The script contains the following input parameters:
\begin{enumerate}
	\item the name of the file \texttt{gathered\_POSCARS}, which is a collection of crystal structures in the POSCAR format
	\item the name of the \emph{spectrum file}, containing angles and intensities of the experimental powder diffraction pattern
	\item the start and end angles for the computation of theoretical diffraction patterns
	\item an array of coefficients $k$ for performing isotropic deformations of the unit cell for all crystal structures
	...
	\item the parameter \emph{th\_peaks\_penalty}, which controls the additional penalty given to non-matching theoretical peaks in our fitness function
\end{enumerate}
The script performs the following operations:
\begin{enumerate}
	\item for each structure, it reads the geometry from \texttt{gathered\_POSCARS}
	\item for each given value of \emph{k}, it performs an isotropic deformation of the unit cell by multiplying each lattice parameter by \emph{k}
	\item it computes the theoretical X-ray diffraction pattern of the so-obtained structure
	\item it computes the disagreement F between theoretical and experimental diffraction patterns using our fitness function
	\item for each structure and for the lowest value of F across all the coefficients \emph{k}, it outputs a CIF file with the symmetrized structure (tolerance 0.2~\AA)
	\item for each structure and for the lowest value of F across all the coefficients \emph{k}, it outputs a PNG graph with the theoretical and experimental diffraction patterns superimposed for comparison
	\item for each structure, it outputs a PNG graph which shows the variation of F with \emph{k}
\end{enumerate}
The name of each CIF and PNG file have the format \texttt{F\_ID\_iupacformula\_k\_ \\ symmetry}, where:
\begin{description}
	\item[F] is the disagreement F between theoretical and experimental diffraction patterns
	\item[ID] is the comment provided in the first line of each POSCAR (if the POSCAR file is produced by USPEX, this is the structure ID in the USPEX run)
	\item[iupacformula] is the \emph{IUPAC formula} of the structure
	\item[k] is the value of the coefficient \emph{k} which gave the lowest disagreement F
	\item[symmetry] is the space group number, determined with a tolerance of 0.2~\AA
\end{description}
...
\begin{enumerate}
	...
	\item a number of intervals in degrees, defining the exclusion region
	\item the extremes of the generated X-ray diffraction pattern in degrees
\end{enumerate}
The script works in a folder with many CIF files, and performs the following:
\begin{enumerate}
	\item it opens, one by one, all CIF files and it predicts the X-ray diffraction pattern of the structure according to the given wavelength
...
\end{enumerate}
\textbf{Example}: \texttt{python exclusion.py --wavelength 0.6199 --cutoff 25 \\ --lextremes 25,31 --rextremes 28,32 --minangle 5 --maxangle 60}
...
\begin{enumerate}
	\item the wavelength of the incident radiation in \SI{}{\angstrom}
	\item the peak cut-off, in \% of the maximum intensity
	\item a number of search intervals in degrees, defining the search region
	\item the extremes of the generated X-ray diffraction pattern in degrees
\end{enumerate}
The script works in a folder with many CIF files, and performs the following:
\begin{enumerate}
...
\end{enumerate}
\textbf{Example}: \texttt{python find\_peak.py --wavelength 0.6199 --cutoff 15 \\ --lextremes 25,31 --rextremes 28,32 --minangle 5 --maxangle 60}

\section{change\_pressure.py}
It translates the structures contained in many CIF files to a different pressure, by deforming the lattice parameters using a second order Taylor expansion of the Birch-Murnaghan equation of state. More details about the underlying theory can be found in appendix \ref{sec:birchmurnaghan}. The script takes the following arguments from command line:
\begin{enumerate}
...
We developed a code which computes, from the experimental powder diffraction pattern and a set of crystal structures, the degree of disagreement (fitness) of each structure with the experimental data.

In the case of powder X-ray diffraction patterns, for each peak we know the diffraction angle and the relative intensity, while the intensity of the highest peak in each spectrum has been conventionally given the value 100. We define a \emph{match} between a theoretical and an experimental peak the case when the two peaks are less than \emph{match\_tol} degrees apart, regardless of their intensities. The fitness between a calculated powder X-ray diffraction pattern and the experimental diffraction pattern is defined by the following fitness function
\begin{equation*}
	F = \sum_{i, j}^{match} \left( \frac{h_i^{exp} - h_j^{th}}{100} \right)^2 \left( \frac{h_i^{exp}}{100} \right)^2 + \sum_i^{exp \, rest} \left( \frac{h_i^{exp}}{100} \right)^2 + p^{th} \cdot \sum_i^{th \, rest} \left( \frac{h_i^{th}}{100} \right)^2
\end{equation*}
where the $h_i$ are diffraction intensities. The first term of the fitness is a sum over the matched peaks, and each addend of this sum will be smaller the more the intensities of the two matched peaks are close to each other. The second term is a sum over the experimental peaks that are left after our matching, we call these peaks \emph{experimental rest}. The third term, analogously, is a sum over the \emph{theoretical rest}. The user-defined coefficient $p^{th} > 1$ which appears in the last term introduces an additional penalty for non-matching theoretical peaks. It is clear that a low value of F gives a good agreement between calculated and experimental powder X-ray diffraction patterns, allowing a quick identification of promising candidates structures.

\section{Single crystal spectra comparison}
\label{sec:scspectracomp}

In the case of single crystal X-ray diffraction patterns, for each reflection we know the Miller indices $(h, k, l)$, its intensity $I$ and its uncertainty of measure~$\sigma$. As fitness we use the weighted R-factor, given by
\[
...
\section{Pressure change}
\label{sec:birchmurnaghan}

It often happens that the USPEX calculation and the experimental data are not exactly at the same pressure, but the two pressures are \emph{close}, that is, no more than \SI{20}{GPa} apart. In this case, our script \texttt{change\_pressure.py} can be used to translate the theoretical structures to the experimental pressure.

We start from the Birch-Murnaghan equation of state
\begin{equation*}
...
Assuming that, for small pressure variations, $V_0/V$ is close to 1, we can approximate the Birch-Murnaghan equation of state to a second order Taylor expansion in $V_0/V$ and get the volume $V$ at the experimental pressure:
\begin{equation*}
...
\end{equation*}
that will be used to rescale the lattice parameters of all the calculated structures. After this rescaling, the relaxed structures are symmetrized with a tolerance of 0.2 \SI{}{\angstrom} and the corresponding CIF files are written.
